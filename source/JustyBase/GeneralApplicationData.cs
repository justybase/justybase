...
public sealed partial class GeneralApplicationData : IGeneralApplicationData
{
    ...
    /// <summary>
    /// Update/plugin feed URL support has been removed.
    /// This property now always resolves to an empty string.
    /// </summary>
    public string DownloadPluginsBasePath => string.Empty;
    ...
    public GeneralApplicationData(IMessageForUserTools messageForUserTools, IOtherHelpers otherHelpers, ISimpleLogger simpleLogger, IEncryptionHelper encryptionHelper)
    {
        ...
        if (!Directory.Exists(IGeneralApplicationData.LogsPath))
        {
            Directory.CreateDirectory(IGeneralApplicationData.LogsPath);
        }
    ...
    public string GetFormatterSql(string txt)
    {
        return NzSqlDocumentFormatter.Format(txt);
    }
}
