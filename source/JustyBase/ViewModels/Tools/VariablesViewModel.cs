...
public partial class VariablesViewModel : Tool, IDisposable
{
    ...
    public VariablesViewModel(IFactory factory, IGeneralApplicationData generalApplicationData,
        IActiveDocumentManager activeDocumentManager)
    {
        ...
        RefreshAllVariables();
        Task.Run(VariableRefresh).ContinueWith(static x => _ = x.Exception, TaskContinuationOptions.OnlyOnFaulted);
    }
    ...
    {
        var dictionary = _generalApplicationData.VariablesDictionary;
        dictionary.Clear();
        foreach (var item in VariableList)
        {
